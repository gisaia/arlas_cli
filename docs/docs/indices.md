...
To be explored in ARLAS dashboards, the data has to be indexed in an ES index. An index contains the data and a mapping to describe how fields have to be interpreted (types).
...
!!! tip
    For NDJSON files generated with Apache Spark, files are often partitioned into several parts into a `file.json` folder:
    ```
    part-00000-[...].json
    part-00001-[...].json
    ...
    ```
    In practice, the `file` Argument can be filed with a pattern such as `path/to/file.json/part-00000-*.json` to avoid writing the generated hash
...
!!! note "--field-mapping"
    ...
    It has the structure **field_name:field_type**

    Examples:

    - `--field-mapping field_point:geo_point`
    - `--field-mapping field_time:date-epoch_second`
